...
if [[ "$OPERATIONTYPE" == "start" ]]; then
    ...
    if [[ "$NODETYPE" == "node"* ]]; then
        $FILE/geth --nousb --datadir=$DATADIR --syncmode 'full' --bootnodes "enode://$BOOTNODEID@$BOOTNODEIP:$BOOTNODEPORT" --networkid $NETWORKID --port $MYNODEPORT --http --http.addr '0.0.0.0' --http.port 8006  --http.api admin,eth,net,txpool,web3  --allow-insecure-unlock --unlock $adressAccount  --password .accountpassword  --http.vhosts="*" --rpcaddr='*'
